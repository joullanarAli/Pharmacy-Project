...
namespace PharmacyWeb.Controllers
{
    ...
    public class ChoicesController : BaseController
    {
        ...
        public async Task<IActionResult> UpdateChoice(int choiceId,Choice choice)
        {
            try
            {
                Choice _choice = await _unitOfWork._choiceRepository.GetById(choiceId);
                _choice.Mark=choice.Mark;
